...
interface CountUpProps {
  ...
  startWhen?: boolean;
  separator?: string;
  showPlus?: boolean; // New prop to control "+" display
  onStart?: () => void;
  onEnd?: () => void;
}
...
export default function CountUp({
  ...
  startWhen = true,
  separator = "",
  showPlus = false, // Default to false if not provided
  onStart,
  onEnd,
}: CountUpProps) {
  ...
  useEffect(() => {
    const unsubscribe = springValue.on("change", (latest) => {
      if (ref.current) {
        ...
        // Set text content with optional "+" suffix
    ...
    return () => unsubscribe();
  }, [springValue, separator, showPlus]);
